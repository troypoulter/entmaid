...
import (
	...
	"github.com/troypoulter/entmaid/examples/basic/predicate"
...
func (uu *UserUpdate) check() error {
	if v, ok := uu.mutation.Age(); ok {
		if err := user.AgeValidator(v); err != nil {
			return &ValidationError{Name: "age", err: fmt.Errorf(`basic: validator failed for field "User.age": %w`, err)}
		}
	}
	return nil
}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (_node *User, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`basic: missing "User.id" for update`)}
	}
	...
	if fields := uuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !user.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("basic: invalid field %q for query", f)}
			}
			if f != user.FieldID {
