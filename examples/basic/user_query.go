...
func (uq *UserQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("basic: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range uq.inters {
		if inter == nil {
			return fmt.Errorf("basic: uninitialized interceptor (forgotten import basic/runtime?)")
		}
	...
	for _, f := range uq.ctx.Fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("basic: invalid field %q for query", f)}
		}
	}
	if uq.path != nil {
